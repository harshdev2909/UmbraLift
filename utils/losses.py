...
class L_spa(nn.Module):
    ...
    def __init__(self):
        ...
        # Directional kernels for edge detection. Registered as buffers so they
        # follow the module across devices without being treated as parameters.
...
class L_texture(nn.Module):
    ...
    def __init__(self):
        ...
        # Sobel filters as buffers, so they move with the module rather than
        # being rebuilt on a hardcoded device every forward pass.
    ...
    def forward(self, img_lowlight, img_enhanced):
        ...
        # Calculate gradients
        grad_low_x = F.conv2d(img_lowlight_gray, self.sobel_x, padding=1)
        grad_low_y = F.conv2d(img_lowlight_gray, self.sobel_y, padding=1)
        grad_enhanced_x = F.conv2d(img_enhanced_gray, self.sobel_x, padding=1)
        grad_enhanced_y = F.conv2d(img_enhanced_gray, self.sobel_y, padding=1)
